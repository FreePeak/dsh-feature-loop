...
| 34 | **§2.6** — boot the hermetic server, expect `dsh web: …token=…` | Token line | **Confirmed** on port 3096 under `DSH_HOME=~/.dsh-qa-verify`: `dsh web: http://127.0.0.1:3096/?token=<redacted>`; tokenless `/` → **401**; zero load errors. | **PASS** | — |
