...
import type { FeatureLoopPolicy } from './agent.ts'
import { DEFAULT_MAX_PARALLEL_TOOL_CALLS } from './constants.ts'
/* FORK-DELTA: the policies this fork adds, built once per plugin load. */
import { LoopBudget } from './budget.ts'
import { ModelLadder } from './routing.ts'
import { AttentionRouter, ReviewGate } from './review.ts'
...
export interface Config {
  ...
  maxParallelToolCalls?: number
  /**
   * FORK-DELTA: the book's 8-dimension loop spec. Supplying it turns on the
   * step and cost ceilings, the cheap-first ladder, and the review router.
   *
   * Omitted means the loop runs exactly as upstream does — unbounded, on one
   * route — which stays supported on purpose: a fork that cannot be turned back
   * into its parent is a fork you cannot compare against its parent.
   */
  spec?: LoopSpec
  /**
   * FORK-DELTA: the local judge that ranks which steps deserve your review.
   * Omitted means detection-only, which is a supported mode, not a broken one.
   */
  judge?: {
    /** onegw base URL. The judge is always reached through the gateway, never directly. */
    baseURL: string
    /** Model alias onegw routes to, e.g. `laya`. */
    model: string
    /** Bearer key for a hosted backend; local Laya ignores it. */
    apiKey?: string
    /** Per-call deadline. Defaults to 2000 ms, above Laya's ~73 ms warm p50. */
    timeoutMs?: number
  }
  /** FORK-DELTA: attention-router overrides. */
  review?: {
    confidenceThreshold?: number
    judgeThreshold?: number
    reviewBudget?: number
  }
  /** FORK-DELTA: per-tool gate policy overrides, by tool name. */
...
/**
 * FORK-DELTA: turn the config into the per-agent policies.
 *
 * Built once per plugin load rather than per agent, and the spec is validated
 * here — at load time — because the book's argument for naming the eight
 * dimensions first is that a missing one is invisible until it has already cost
 * money. A loop that starts without a cost budget cannot be given one
 * retroactively.
 *
 * @param config - the plugin's resolved config.
 * @returns the policy handed to every agent this factory creates, and the
 *   router whose review budget spans the deployment.
 */
function buildPolicy(config: Config): { policy: FeatureLoopPolicy, router: AttentionRouter | undefined } {
  if (config.spec === undefined) {
    // Upstream behaviour, deliberately preserved: no ceilings, one route.
    return { policy: {}, router: undefined }
  }
  const spec: LoopSpec = validateSpec(config.spec)
  ...
  const router = new AttentionRouter(config.review ?? {})
  const judge: Judge | undefined = config.judge === undefined
    ? undefined
    : new OnegwJudge({
      baseURL: config.judge.baseURL,
      model: config.judge.model,
      apiKey: config.judge.apiKey,
      timeoutMs: config.judge.timeoutMs ?? 2000,
    })
  return {
    policy: {
      budget,
      ...
      router,
      judge,
      gate: new ReviewGate((config.gatePolicies ?? {}) as Record<string, GatePolicy>),
      spec,
    },
    router,
  }
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  static Config = z.object({
    ...
    /* FORK-DELTA: the policy surface. All optional — omitting every one of them
     * reproduces upstream behaviour exactly, which is what keeps this fork
     * comparable against its parent. */
    spec: z.any(),
    judge: z.object({
  ...
  /** FORK-DELTA: the policies every agent this factory creates inherits. */
  private readonly policy: FeatureLoopPolicy
  /** FORK-DELTA: the attention router, shared so its review budget spans the run. */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    // FORK-DELTA: build the policies once; a spec problem throws here, at load,
    // rather than at the first step that spends money.
    const built = buildPolicy(this.config)
    this.policy = built.policy
