...
import type { GenerateOptions, LlmCallConfig, Message, MessageSource, PreparedLlmCall, TokenUsage } from '@deepseek-ai/dsh-llm'
import {
  LlmError,
...
export interface FeatureLoopPolicy {
  /** Step and cost ceilings. Omitted means unbounded — the upstream behaviour. */
  budget?: LoopBudget
  /** Cheap-first model ladder. Omitted means the session's own single route. */
  ladder?: ModelLadder
  /** Attention router: which steps deserve your review, under a budget. */
  router?: AttentionRouter
  /** The local judge that ranks review-worthiness. Omitted means detection-only. */
  judge?: Judge
  /** The reversibility-tiered safety gate. */
  gate?: ReviewGate
  /** The validated 8-dimension spec this run obeys. */
  spec?: LoopSpec
}
...
export class ReactLoopAgent implements Agent {
  ...
  /* FORK-DELTA(0): policy state. All of it is absent in upstream, where the
   * loop runs unbounded on one route for the whole session. */
  /** Ceilings and the price table, when the deployment configured them. */
  private readonly budget: LoopBudget | undefined
  /** The cheap-first ladder, when the deployment configured one. */
  private readonly ladder: ModelLadder | undefined
  /** Cost of the most recent settled attempt, fed to the ladder's per-step cap. */
  private lastAttemptUSD: number | undefined
  /** Whether the one-step handoff has already been granted at a ceiling. */
  private stopNoticeIssued = false
  /** Whether the convergence nudge has already been sent. */
  private warnNoticeIssued = false
  /** The route label in use, so an escalation can be announced once. */
  private announcedRoute: string | undefined
  ...
  /**
   * FORK-DELTA(4): price one settled model attempt into the run's budget.
   *
   * An unpriceable route is logged and tolerated rather than thrown: the budget
   * is the thing that should fail, never the agent. But it is never silent — an
   * unlogged pricing failure would make the ceiling a fiction, which is worse
  ...
  private async preStep(target: InboxTarget, position: { turn: number; step: number }): Promise<PreparedStep> {
    ...
    /* FORK-DELTA(1): step and cost ceilings.
     *
     * Upstream has no turn budget — its README lists "No built-in turn budget"
     * as a known limitation — so this is the fork's whole reason to exist.
     *
     * It runs BEFORE the model call that would breach the ceiling, which is
     * what makes the ceiling a limit rather than an invoice. A stop grants
     * exactly one further step so the run ends with a labelled handoff instead
     * of an empty halt; the step after that is rejected outright.
     */
    let messages = decision.messages
    if (this.budget !== undefined) {
      const verdict = this.budget.verdict(position.step)
      if (verdict.kind === 'stop') {
        if (this.stopNoticeIssued) return { kind: 'reject' }
        this.stopNoticeIssued = true
        messages = [...messages, budgetStopMessage(verdict.reason)]
      } else if (verdict.kind === 'warn' && !this.warnNoticeIssued) {
     ...
     * `prepareRequest`.
     *
     * The route is chosen at the step boundary, not at request assembly,
     * because a rung change is something the model should be told about — and
     * the only channel that reaches the model is the message batch decided
     * here. Deciding in `prepareRequest` would move the model silently.
     *
     * One route per step, stable across that step's retries: a retry is the
     * same work, so it must not walk the ladder by itself.
     */
    if (this.ladder !== undefined) {
      const routed = this.ladder.forStep(position.step, this.lastAttemptUSD)
      const label = routeLabel(routed.route)
      if (this.announcedRoute !== undefined && this.announcedRoute !== label) {
        const notice = escalationMessage(this.announcedRoute, label, routed.reason.kind)
        if (notice !== undefined) messages = [...messages, notice]
      }
      this.announcedRoute = label
      this.stepRoute = routed.route
    }
    if (messages !== decision.messages) return { ...decision, messages, assembly }
      ...
      try {
        ...
        try {
          if (signal.aborted) {
            ...
            // FORK-DELTA(4): an aborted attempt was still billed by the provider.
            // Metering only the completed path would under-report every run the
            // budget stopped — the runs the budget exists for.
  ...
  private async prepareRequest(
    ...
    /* FORK-DELTA(3): apply the step's routed rung to the request seed.
     *
     * Upstream seeds the waterfall with the persisted header, which pins one
     * route for the whole session. The fork substitutes the rung `preStep`
     * chose, and still runs the `agent/request` waterfall afterwards so an
     * operator listener keeps the final word — a ladder the deployment cannot
     * override would be a policy it does not own.
     */
    const routedSeed: LlmCallConfig = this.stepRoute === undefined
      ? seedConfig
      : deepFreeze(structuredClone({
        ...seedConfig,
        model: this.stepRoute.model,
        ...this.stepRoute.provider === undefined ? {} : { provider: this.stepRoute.provider },
        ...this.stepRoute.reasoningEffort === undefined
          ? {}
          : { reasoningEffort: this.stepRoute.reasoningEffort as LlmCallConfig['reasoningEffort'] },
      }))
    const proposedConfig = await this.dispatch.waterfall(
      'agent/request', { turn, step, signal },
      () => Promise.resolve(routedSeed),
    )
    signal.throwIfAborted()
